...
its **real root filesystem** on an Intel Mac — no Apple Silicon, no device — using
[`jprx/darwin-vm`](https://github.com/jprx/darwin-vm) and its `qemu-sptm` fork, and
lighting the **DCP display panel** with the live boot log along the way.
...
> **Status:** the full OS boots through SPTM → XNU, **mounts its real APFS root**, and
...
| ![boot screen](shots/panel-boot-screen.png) | **Graphical boot screen** — progress ring + stage label driven by real boot milestones scraped from the serial log. |
| ![console](shots/panel-ios-console.png) | **Live kernel console** rendered on the panel with a built-in VGA font. |
| ![root shell](shots/panel-root-shell.png) | **Interactive root shell** (`bash-5.3#`) — panel + keyboard wired to the guest UART. |
...
|---|:--:|
| SPTM/TXM → XNU boot | ✅ |
| Guarded-domain (SK/CL4) executes | ✅ |
| DCP panel lit (scanout) | ✅ |
| Real boot log on panel + progress | ✅ |
| Interactive root shell on panel | ✅ |
| Full rootfs boot → APFS `mountroot` | ✅ |
| `md0` >4 GB truncation fix | ✅ |
| `/sbin/launchd` starts | ✅ |
| `libSystem` / dyld shared cache | ⛔ needs SystemOS **Cryptex** |
| IOMFB real-surface decode | 🚧 |
...
See [`docs/SIGUIENTE-PASO-cryptex.md`](docs/SIGUIENTE-PASO-cryptex.md).
...
DARWIN_RTKIT=1 DARWIN_FB=1 ./run/ver_pantalla.sh
...
run/          ver_pantalla (panel), run_rootfs (full OS), VNC live-view
