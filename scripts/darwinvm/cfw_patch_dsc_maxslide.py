...
def _cli():
    ...
    if not args:
        print("usage: cfw_patch_dsc_maxslide.py <cache_dir_with_dyld_shared_cache_arm64e> [--dry-run] [--force]")
        sys.exit(1)
    n = patch_dsc_maxslide(args[0], dry_run="--dry-run" in sys.argv, force="--force" in sys.argv)
